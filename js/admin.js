...
async function loadResults() {
  ...
  try {
    ...
    // Fetch student names for all emails
    const studentEmails = Object.keys(groupedResults);
    const studentNamesMap = {};
    await Promise.all(studentEmails.map(async email => {
      try {
        const studentDoc = await getDoc(doc(db, "students", email));
        studentNamesMap[email] = studentDoc.exists() ? studentDoc.data().studentName : email;
      } catch (err) {
        console.warn("Error fetching student name for", email, err);
        studentNamesMap[email] = email;
      }
    }));

    // Sort students by latest submittedAt (descending)
    const sortedStudents = studentEmails.sort((a, b) => {
    ...
    sortedStudents.forEach(studentEmail => {
      const studentName = studentNamesMap[studentEmail] || studentEmail;
      const card = document.createElement("div");
      card.classList.add("result-card");

      card.innerHTML = `
        <div class="result-card-header">
          <span class="result-card-title" title="${studentEmail}">${studentName}</span>
      ...
      // Sort attempts by submittedAt (oldest → newest)
      groupedResults[studentEmail]
        ...
        .forEach((r, index) => {
          ...
          // Swipe-to-delete logic remains unchanged
          (function attachSwipeHandlers(attemptEl, card, body) {
            let startX = 0, startY = 0, isPointerDown = false, isSwiping = false;
            const THRESHOLD = 120;
            attemptEl.style.touchAction = "pan-y";
            attemptEl.style.willChange = "transform";

            attemptEl.addEventListener("pointerdown", (e) => {
              if (e.pointerType === "mouse" && e.button !== 0) return;
              startX = e.clientX; startY = e.clientY;
              isPointerDown = true; isSwiping = false;
            ...
            attemptEl.addEventListener("pointermove", (e) => {
              ...
              if (!isSwiping) {
                if (Math.abs(dx) > 8 && Math.abs(dx) > Math.abs(dy)) isSwiping = true;
                else if (Math.abs(dy) > 10 && Math.abs(dy) > Math.abs(dx)) {
                  isPointerDown = false;
                  try { attemptEl.releasePointerCapture(e.pointerId); } catch { }
              ...
              if (isSwiping) {
                if (dx > 0) {
                  const translate = Math.min(dx, window.innerWidth);
                  attemptEl.style.transform = `translateX(${translate}px)`;
                  attemptEl.classList.toggle("dragging", translate > THRESHOLD * 0.6);
                } else {
            ...
            attemptEl.addEventListener("pointerup", async (e) => {
              ...
              if (isSwiping && dx > THRESHOLD) {
                attemptEl.style.transform = `translateX(100%)`; attemptEl.style.opacity = "0"; attemptEl.classList.remove("dragging");
                setTimeout(async () => {
                  try {
                    await deleteDoc(doc(db, "results", attemptEl.dataset.id));
                    attemptEl.remove();
                    Array.from(body.querySelectorAll(".attempt-card")).forEach((el, idx) => {
                      const h4 = el.querySelector("h4"); if (h4) h4.textContent = `Attempt ${idx + 1} — ${el.dataset.testTitle || ""}`;
                    });
                    if (body.children.length === 0) card.remove();
                    else if (card.classList.contains("open")) body.style.maxHeight = body.scrollHeight + "px";
                  } catch (err) {
                    console.error("Error deleting attempt:", err);
                    attemptEl.style.transform = "translateX(0)"; attemptEl.style.opacity = "1";
                  }
                }, 260);
              } else {
                attemptEl.style.transform = "translateX(0)"; attemptEl.classList.remove("dragging");
              }
            });
      ...
      header.addEventListener("click", () => {
        document.querySelectorAll(".result-card.open").forEach(other => {
          if (other !== card) {
            ...
            const b = other.querySelector(".result-card-body"); if (b) b.style.maxHeight = null;
            const i = other.querySelector(".toggle-icon"); if (i) i.classList.replace("fa-chevron-up", "fa-chevron-down");
          }
        });
        card.classList.toggle("open");
        if (card.classList.contains("open")) { body.style.maxHeight = body.scrollHeight + "px"; icon.classList.replace("fa-chevron-down", "fa-chevron-up"); }
        else { body.style.maxHeight = null; icon.classList.replace("fa-chevron-up", "fa-chevron-down"); }
      });
